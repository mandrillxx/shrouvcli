...
export function Gradient({ Color, Rotation, Transparency }: IGradient) {
  return (
    <uigradient
      ...
      Transparency={Transparency ?? new NumberSequence(0)}
    />
  );
}
...
export function Stroke({ Thickness }: IStroke) {
  return (
    <uistroke Thickness={Thickness ?? 3} Color={Color3.fromRGB(0, 0, 0)} />
  );
}
...
export function Button({
  ...
  return (
    ...
    >
      ...
      <Text
        Size={UDim2.fromScale(0.463, 0.754)}
        Text={ButtonText}
  ...
  const { size } = useSpring(
    {
      ...
      size: hovering
        ? UDim2.fromScale(Size.X.Scale * 1.05, Size.Y.Scale * 1.15)
        : Size,
    },
    [hovering]
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text
          Text={`${Progress}/${Maximum ?? 100}`}
          Size={UDim2.fromScale(0.08, 1.161)}
        />
      </frame>
    </Frame>
  );
...
export function BackgroundBlur({ blurSize }: IBackgroundBlur) {
  ...
  return (
    <Roact.Portal target={camera}>
      {visible && <blureffect Size={blurSize} />}
    </Roact.Portal>
  );
}
